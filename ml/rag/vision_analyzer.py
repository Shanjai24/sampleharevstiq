...
class VisionAnalyzer:
    ...
    def analyze_image(self, image_b64, crop_hint=None):
        ...
        if gemini_key and gemini_key.startswith('AIza'):
            try:
                # Direct Gemini 2.0 Flash REST endpoint with inline data payload
                url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key={gemini_key}"
                payload = {
                    "contents": [{
                        "parts": [
                            {"text": SYSTEM_VISION_PROMPT + (f"\nFarmer note/crop hint: {crop_hint}" if crop_hint else "")},
                            {
                                "inline_data": {
        ...
        return {
            ...
            'confidence': 0.85,
            'title': 'Yellow Leaf Spot / Early Blight Suspected',
            'crop': crop_hint.capitalize() if crop_hint else 'Crop Foliage',
            'growthStage': 'Vegetative / Flowering Stage',
            'description': 'Visible chlorotic yellow spots and mild leaf curl observed on upper foliage, characteristic of early fungal leaf spot or nutrient stress.',
            'cause': 'High ambient humidity combined with dew retention on leaf surfaces.',
            'idealConditions': 'Temperatures 24-32°C, well-aerated soil, balanced NPK ratio.',
            'steps': {
                'immediate_organic': [
                    'Spray Neem Cake Extract (5%) or Neem Oil @ 5ml/litre water every 7 days.',
                    'Apply Bio-fungicide Trichoderma viride @ 5g/litre water on affected foliage.'
                ],
                'chemical_options': [
                    'Spray Mancozeb 75% WP @ 2g/litre water or Copper Oxychloride @ 3g/litre water.',
